    ...
    if args.mode == "process":
        ...
        for row in sample_df.itertuples():
            ...
            if args.reference == None:
                align_command = f"chip_align.py align -read {read} -mode {align_mode}"
            else:
                print(f"Reference: {args.reference}")
                align_command = f"chip_align.py align -read {read} -ref {args.reference} -mode {align_mode}"
            if args.force:
                align_command += " -f "
            print(align_command)
            subprocess.call(align_command, shell = True)
            # markdup step 
            align_bam = f"/data/jim4/Seq/CRISPR/alignment/individual/{row.id}.bam"
            markdup_command = f"chip_align.py markdup -bam {align_bam}"
            if args.force:
                markdup_command += " -f"
            print(markdup_command)
            subprocess.call(markdup_command, shell = True)
            # markdup 
            mark_bam = f"/data/jim4/Seq/CRISPR/alignment/markdup/{row.id}.bam"
            bw_command = f"chip_align.py bigwig -bam {mark_bam}"
            if args.force:
                bw_command += " -f"
            print(bw_command)
